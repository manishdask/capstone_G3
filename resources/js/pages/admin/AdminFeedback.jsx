...
// Simple rating-based grouping for display only — FR47 (AI sentiment
// classification) is out of scope; this is not an AI feature.
function getSentiment(rating) {
  if (rating >= 4) return { label: "Positive", color: "var(--sage)" };
  if (rating === 3) return { label: "Neutral", color: "var(--muted)" };
  return { label: "Negative", color: "var(--rose)" };
}

function SentimentBadge({ rating }) {
  const s = getSentiment(rating);
  return (
    <span className="f-body" style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "3px 9px", borderRadius: 999, fontSize: 11, fontWeight: 700, background: `${s.color}18`, color: s.color, border: `1px solid ${s.color}44` }}>
    ...
    <div className="rise">
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        <div>
          ...
          <div className="f-body" style={{ fontSize: 13, color: "var(--muted)" }}>Patient reviews and ratings</div>
        </div>
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 14 }}>
        {["All", "Positive", "Neutral", "Negative"].map((f) => (
          <button key={f} onClick={() => setFilter(f)} className="f-body" style={{ padding: "6px 13px", borderRadius: 999, fontSize: 12, fontWeight: 600, border: "1px solid var(--line)", cursor: "pointer", background: filter === f ? "var(--ink)" : "#fff", color: filter === f ? "#fff" : "var(--ink-deep)" }}>
            {f}
          </button>
        ))}
      </div>
      ...
      {feedbacks.length === 0 ? (
        <Card style={{ textAlign: "center", padding: 28 }}>
          ...
          <div className="f-body" style={{ fontSize: 13, color: "var(--muted)" }}>No patient feedback submitted yet.</div>
        </Card>
      ) : filtered.length === 0 ? (
        <Card style={{ textAlign: "center", padding: 20 }}>
          <div className="f-body" style={{ fontSize: 13, color: "var(--muted)" }}>No {filter.toLowerCase()} feedback entries.</div>
        </Card>
      ) : (
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                    {f.doctor && f.doctor !== "General" && (
                      <div className="f-body" style={{ fontSize: 11, color: "var(--muted)" }}>Doctor: <strong>{f.doctor}</strong></div>
                    )}
                    <div className="f-body" style={{ fontSize: 11, color: "var(--muted)" }}>Branch: <strong>{f.branch}</strong></div>
                  </div>
                </div>
                <div style={{ textAlign: "right", flexShrink: 0, marginLeft: 12 }}>
                  <div className="f-body" style={{ fontSize: 11.5, fontWeight: 600, color: "var(--ink-deep)" }}>{f.patientName}</div>
                  <div className="f-mono" style={{ fontSize: 10.5, color: "var(--muted)" }}>{f.patientId}</div>
                  <div className="f-body" style={{ fontSize: 10.5, color: "var(--muted)", marginTop: 3 }}>{f.date} · {f.time}</div>
                </div>
              </div>
            </Card>
          ))}
