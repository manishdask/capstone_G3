...
export default function Login() {
  ...
  const [reg, setReg] = useState({
    ...
    dob: "", gender: "", contact: "", allergies: "", branchId: DEFAULT_BRANCH_ID,
  });
  ...
  async function handleRegister(e) {
    ...
    try {
      await register({
        ...
        branch_id: Number(reg.branchId) || DEFAULT_BRANCH_ID,
      });
    } catch (err) {
        ...
        <div style={{ width: "100%", maxWidth: 390 }}>
          ...
          {tab === "register" && (
            <form onSubmit={handleRegister}>
              ...
              <div className="sgh-field">
                <label className="sgh-label">Preferred Branch</label>
                <select className="sgh-input" value={reg.branchId} onChange={(e) => setField("branchId", Number(e.target.value))}>
                  {branches.length > 0 ? (
                    branches.map((b) => (
                    ...
                    <option value={DEFAULT_BRANCH_ID}>Main Hospital (NSW)</option>
                  )}
                </select>
              </div>
