  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", flexWrap: "wrap" }}>
      ...
      <div
        ...
        style={{
          flex: "1.2 1 500px",
          background: "linear-gradient(135deg, var(--ink-deep) 0%, #154641 100%)",
          color: "#fff",
          padding: "50px 48px",
          display: "flex",
      ...
      >
        {/* Visual ECG heartbeat decoration in background */}
        <div style={{ position: "absolute", bottom: "10%", right: "-10%", opacity: 0.05, transform: "scale(2.5)" }}>
          <PulseDivider color="#fff" width={300} height={120} />
        </div>

        <div>
          {/* Logo */}
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 28 }}>
            <PulseDivider color="var(--amber)" width={54} height={22} />
            <span className="f-display" style={{ fontWeight: 700, fontSize: 16, letterSpacing: 0.5 }}>
              ST GEORGE HOSPITAL
            </span>
          </div>

          {/* Title */}
          <div className="f-display" style={{ fontSize: 32, fontWeight: 700, lineHeight: 1.2, maxWidth: 500 }}>
            Centralised Healthcare Management Platform
          </div>

          <div className="f-body" style={{ fontSize: 13.5, color: "#CFE0DC", marginTop: 12, maxWidth: 460, lineHeight: 1.6 }}>
            A secure client–server solution designed for St. George Hospital Group (Australia). Unifies multi-state branches, patient history recording, real-time consultation scheduling, ward admissions, lab diagnostics, billing, and pharmacy stock controls.
          </div>

          {/* Academic Banner Credits */}
          <div
            style={{
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
              ...
              <span className="f-display" style={{ fontSize: 13.5, fontWeight: 700 }}>
                Capstone Project · CPRO306 (Group G3)
              </span>
            </div>
            
            <div className="f-body" style={{ fontSize: 12, color: "#CFE0DC", lineHeight: 1.5 }}>
              <strong>Executive Sponsor:</strong> Mr. Mehedi Hasan <br />
              <strong>Team Members:</strong>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "4px 12px", marginTop: 6, fontFamily: "monospace", fontSize: "11.5px" }}>
                <span>• Manish Das (K240885)</span>
                <span>• Priya Paneru (K241081)</span>
                <span>• Krishala Niroula (K240986)</span>
                <span>• Usha Tamang (K240963)</span>
                <span>• Oyndrila Saha (K240247)</span>
              </div>
            </div>
          </div>
        </div>

        {/* Security & Regulatory Info */}
        <div style={{ display: "flex", flexDirection: "column", gap: 14, marginTop: 30 }}>
          {[
            { icon: ShieldCheck, title: "Data Security Compliance", text: "Aligned with Australian Privacy Principles (Privacy Act 1988), HIPAA security directives, and NIST guidelines. Features AES data encryption at-rest." },
            { icon: Lock, title: "Access Control & Audits", text: "Role-Based Access Control (RBAC) dynamically locks clinical modules. Automatic logging of logins, clinical modifications, and backups (NFR12)." },
            { icon: Building2, title: "Centralized Multi-Branch Operations", text: "Synchronized branch-wise scheduling and reporting. Central backup triggers and bed tracking for NSW & VIC facilities." },
          ].map((f, i) => (
            <div key={i} style={{ display: "flex", gap: 12, alignItems: "flex-start", maxWidth: 480 }}>
              <div style={{ width: 32, height: 32, borderRadius: 9, background: "rgba(255,255,255,.08)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                <f.icon size={16} color="var(--amber)" />
              </div>
              <div>
                <div className="f-display" style={{ fontSize: 12.5, fontWeight: 700, color: "#fff" }}>{f.title}</div>
                <div className="f-body" style={{ fontSize: 11.5, color: "#B9C7C3", lineHeight: 1.4, marginTop: 2 }}>{f.text}</div>
              </div>
            </div>
          ))}
        </div>
          ...
          {tab === "login" && (
            <form onSubmit={handleLogin}>
              ...
              <button
                ...
                className="f-body"
                style={{
                  width: "100%", background: "var(--amber)", color: "var(--ink-deep)", border: "none",
                  padding: "12px 0", borderRadius: 12, fontWeight: 700, fontSize: 14, cursor: "pointer", marginTop: 4,
                  boxShadow: "0 4px 10px rgba(242,167,59,.2)",
                }}
              >
                Sign In securely
              </button>
            ...
            {forgotSent ? (
              ...
              <form onSubmit={handleForgot}>
                ...
                <button type="submit" className="f-body"
                  style={{ width: "100%", background: "var(--amber)", color: "var(--ink-deep)", border: "none", padding: "11px 0", borderRadius: 10, fontWeight: 700, fontSize: 13.5, cursor: "pointer", marginTop: 4 }}>
                  Send Reset Link
                </button>
              </form>
            )}
